...
final class ThumbnailProvider: QLThumbnailProvider {
    override func provideThumbnail(for request: QLFileThumbnailRequest, _ handler: @escaping (QLThumbnailReply?, Error?) -> Void) {
        let handlerBox = HandlerBox(call: handler)
        DispatchQueue.global(qos: .userInitiated).async { [handlerBox] in
            let image: CGImage
            ...
            let finalImage = Self.trimForegroundBounds(from: image) ?? image

            // Write to temporary PNG and let the system render it natively
            let tempURL = URL(fileURLWithPath: NSTemporaryDirectory())
                .appendingPathComponent(UUID().uuidString)
                .appendingPathExtension("png")

            guard Self.writePNG(finalImage, to: tempURL) else {
                DispatchQueue.main.async {
                    handlerBox.call(nil, nil)
                }
                return
            }
    ...
    private static func fallbackImage(for fileURL: URL) -> CGImage {
        ...
        // 1x1 transparent pixel as absolute last resort
